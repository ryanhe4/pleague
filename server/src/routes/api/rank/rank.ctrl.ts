...
export const summonerNoParam = async (req: express.Request, res: express.Response) => {
  try {
    ...
    let count
    if (!req.query.cursor) {
      count = await summonerQuery.getCount()
      return res.status(200).send({ summoner, school: school?.school_name, count: Math.ceil(count/3) })
    }
...
export const summonerList = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    if (!req.query.cursor) {
      count = await summonerQuery.getCount()
      return res.status(200).send({ summoner, count : Math.ceil(count/3) })
    }
